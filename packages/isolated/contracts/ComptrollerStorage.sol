...
contract ComptrollerStorage {
    ...
    /// @notice Borrow caps enforced by borrowAllowed for each seToken address. Defaults to zero which restricts borrowing.
    mapping(address => uint256) public borrowCaps;

    /// @notice Minimal collateral required for regular (non-batch) liquidations
    uint256 public minLiquidatableCollateral;

    /// @notice Supply caps enforced by mintAllowed for each seToken address. Defaults to zero which corresponds to minting not allowed
