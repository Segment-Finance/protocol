...
contract RewardsDistributor is ExponentialNoError, Ownable2StepUpgradeable, AccessControlledV8, MaxLoopsLimitHelper {
    ...
    event RewardTokenSupplySpeedUpdated(SeToken indexed seToken, uint256 newSpeed);

    /// @notice Emitted when a new borrow-side REWARD TOKEN speed is calculated for a market
    event RewardTokenBorrowSpeedUpdated(SeToken indexed seToken, uint256 newSpeed);
    ...
    event MarketInitialized(address indexed seToken);

    /// @notice Emitted when a reward token supply index is updated
    event RewardTokenSupplyIndexUpdated(address indexed seToken);

    /// @notice Emitted when a reward token borrow index is updated
    event RewardTokenBorrowIndexUpdated(address indexed seToken, Exp marketBorrowIndex);
    ...
    event SupplyLastRewardingBlockUpdated(address indexed seToken, uint32 newBlock);

    /// @notice Emitted when a reward token last rewarding block for borrow is updated
    event BorrowLastRewardingBlockUpdated(address indexed seToken, uint32 newBlock);
    ...
    function initializeMarket(address seToken) external onlyComptroller {
        uint32 blockNumber = safe32(getBlockNumber(), "block number exceeds 32 bits");

        RewardToken storage supplyState = rewardTokenSupplyState[seToken];
        RewardToken storage borrowState = rewardTokenBorrowState[seToken];
        ...
        emit MarketInitialized(seToken);
    }
     ...
     * @param marketBorrowIndex The current global borrow index of seToken
     */
    function distributeBorrowerRewardToken(
        ...
        _distributeBorrowerRewardToken(seToken, borrower, marketBorrowIndex);
    }

    function updateRewardTokenSupplyIndex(address seToken) external onlyComptroller {
        _updateRewardTokenSupplyIndex(seToken);
    }
    ...
    function updateRewardTokenBorrowIndex(address seToken, Exp memory marketBorrowIndex) external onlyComptroller {
        _updateRewardTokenBorrowIndex(seToken, marketBorrowIndex);
    }

    /**
     * @notice Set REWARD TOKEN borrow and supply speeds for the specified markets
     * @param seTokens The markets whose REWARD TOKEN speed to update
    ...
    function setRewardTokenSpeeds(
        ...
        for (uint256 i; i < numTokens; ++i) {
            _setRewardTokenSpeed(seTokens[i], supplySpeeds[i], borrowSpeeds[i]);
        }
    }

    /**
     * @notice Set REWARD TOKEN last rewarding block for the specified markets
     * @param seTokens The markets whose REWARD TOKEN last rewarding block to update
    ...
    function setLastRewardingBlocks(
        ...
        for (uint256 i; i < numTokens; ) {
            _setLastRewardingBlock(seTokens[i], supplyLastRewardingBlocks[i], borrowLastRewardingBlocks[i]);
            unchecked {
                ++i;
            }
        }
    ...
    function distributeSupplierRewardToken(address seToken, address supplier) external onlyComptroller {
        _distributeSupplierRewardToken(seToken, supplier);
    }
     ...
     * @param holder The address to claim REWARD TOKEN for
     * @param seTokens The list of markets to claim REWARD TOKEN in
     */
    function claimRewardToken(address holder, SeToken[] memory seTokens) public {
        ...
        for (uint256 i; i < seTokensCount; ++i) {
            ...
            require(comptroller.isMarketListed(seToken), "market must be listed");
            Exp memory borrowIndex = Exp({ mantissa: seToken.borrowIndex() });
            _updateRewardTokenBorrowIndex(address(seToken), borrowIndex);
            _distributeBorrowerRewardToken(address(seToken), holder, borrowIndex);
            _updateRewardTokenSupplyIndex(address(seToken));
            _distributeSupplierRewardToken(address(seToken), holder);
        }
     ...
     * @param seToken market's whose reward token last rewarding block to be updated
    ...
    function _setLastRewardingBlock(
        ...
        require(comptroller.isMarketListed(seToken), "rewardToken market is not listed");
        ...
        uint32 currentSupplyLastRewardingBlock = rewardTokenSupplyState[address(seToken)].lastRewardingBlock;
        uint32 currentBorrowLastRewardingBlock = rewardTokenBorrowState[address(seToken)].lastRewardingBlock;
        ...
        if (currentSupplyLastRewardingBlock != supplyLastRewardingBlock) {
            rewardTokenSupplyState[address(seToken)].lastRewardingBlock = supplyLastRewardingBlock;
            emit SupplyLastRewardingBlockUpdated(address(seToken), supplyLastRewardingBlock);
        }

        if (currentBorrowLastRewardingBlock != borrowLastRewardingBlock) {
            rewardTokenBorrowState[address(seToken)].lastRewardingBlock = borrowLastRewardingBlock;
            emit BorrowLastRewardingBlockUpdated(address(seToken), borrowLastRewardingBlock);
        }
    }

    /**
     * @notice Set REWARD TOKEN speed for a single market.
     * @param seToken market's whose reward token rate to be updated
     ...
     */
    function _setRewardTokenSpeed(SeToken seToken, uint256 supplySpeed, uint256 borrowSpeed) internal {
        require(comptroller.isMarketListed(seToken), "rewardToken market is not listed");

        if (rewardTokenSupplySpeeds[address(seToken)] != supplySpeed) {
            ...
            rewardTokenSupplySpeeds[address(seToken)] = supplySpeed;
            emit RewardTokenSupplySpeedUpdated(seToken, supplySpeed);
        }

        if (rewardTokenBorrowSpeeds[address(seToken)] != borrowSpeed) {
            ...
            // Update speed and emit event
            rewardTokenBorrowSpeeds[address(seToken)] = borrowSpeed;
            emit RewardTokenBorrowSpeedUpdated(seToken, borrowSpeed);
        }
    }
     ...
     */
    function _distributeSupplierRewardToken(address seToken, address supplier) internal {
        RewardToken storage supplyState = rewardTokenSupplyState[seToken];
        uint256 supplyIndex = supplyState.index;
        uint256 supplierIndex = rewardTokenSupplierIndex[seToken][supplier];

        // Update supplier's index to the current index since we are distributing accrued REWARD TOKEN
        rewardTokenSupplierIndex[seToken][supplier] = supplyIndex;
        ...
        // Calculate change in the cumulative sum of the REWARD TOKEN per seToken accrued
        Double memory deltaIndex = Double({ mantissa: sub_(supplyIndex, supplierIndex) });

        uint256 supplierTokens = SeToken(seToken).balanceOf(supplier);

        // Calculate REWARD TOKEN accrued: seTokenAmount * accruedPerSeToken
        ...
        emit DistributedSupplierRewardToken(SeToken(seToken), supplier, supplierDelta, supplierAccrued, supplyIndex);
    }
     ...
     * @param marketBorrowIndex The current global borrow index of seToken
     */
    function _distributeBorrowerRewardToken(address seToken, address borrower, Exp memory marketBorrowIndex) internal {
        RewardToken storage borrowState = rewardTokenBorrowState[seToken];
        uint256 borrowIndex = borrowState.index;
        uint256 borrowerIndex = rewardTokenBorrowerIndex[seToken][borrower];

        // Update borrowers's index to the current index since we are distributing accrued REWARD TOKEN
        rewardTokenBorrowerIndex[seToken][borrower] = borrowIndex;
        ...
        // Calculate REWARD TOKEN accrued: seTokenAmount * accruedPerBorrowedUnit
        if (borrowerAmount != 0) {
            ...
            emit DistributedBorrowerRewardToken(SeToken(seToken), borrower, borrowerDelta, borrowerAccrued, borrowIndex);
        }
    }
     ...
     * @dev Index is a cumulative sum of the REWARD TOKEN per seToken accrued
     */
    function _updateRewardTokenSupplyIndex(address seToken) internal {
        RewardToken storage supplyState = rewardTokenSupplyState[seToken];
        uint256 supplySpeed = rewardTokenSupplySpeeds[seToken];
        uint32 blockNumber = safe32(getBlockNumber(), "block number exceeds 32 bits");
        ...
        emit RewardTokenSupplyIndexUpdated(seToken);
    }
     ...
     * @dev Index is a cumulative sum of the REWARD TOKEN per seToken accrued
     */
    function _updateRewardTokenBorrowIndex(address seToken, Exp memory marketBorrowIndex) internal {
        RewardToken storage borrowState = rewardTokenBorrowState[seToken];
        uint256 borrowSpeed = rewardTokenBorrowSpeeds[seToken];
        uint32 blockNumber = safe32(getBlockNumber(), "block number exceeds 32 bits");
        ...
        emit RewardTokenBorrowIndexUpdated(seToken, marketBorrowIndex);
    }
}
