 ...
 * @title SeToken
 * @author Segment
 * @notice Each asset that is supported by a pool is integrated through an instance of the `SeToken` contract. As outlined in the protocol overview,
 * each isolated pool creates its own `seToken` corresponding to an asset. Within a given pool, each included `seToken` is referred to as a market of
 * the pool. The main actions a user regularly interacts with in a market are:

- mint/redeem of seTokens;
- transfer of seTokens;
- borrow/repay a loan on an underlying asset;
- liquidate a borrow or liquidate/heal an account.

 * A user supplies the underlying asset to a pool by minting `seTokens`, where the corresponding `seToken` amount is determined by the `exchangeRate`.
 ...
 * `seToken` in a pool, they can borrow any asset in the isolated pool by using their `seToken` as collateral. In order to borrow an asset or use a `seToken`
 * as collateral, the user must be entered into each corresponding market (else, the `seToken` will not be considered collateral for a borrow). Note that
 ...
 *
 * The Segment protocol includes unique mechanisms for healing an account and liquidating an account. These actions are performed in the `Comptroller`
 ...
 * Both functions settle all of an account’s borrows, but `healAccount()` may add `badDebt` to a seToken. For more detail, see the description of
    ...
    TokenErrorReporter
{
    ...
    /**
     * @notice Sender calls on-behalf of minter. minter supplies assets into the market and receives seTokens in exchange
     * @dev Accrues interest whether or not the operation succeeds, unless reverted
     ...
     * @custom:error LiquidateAccrueCollateralInterestFailed is thrown when it is not possible to accrue interest on the collateral seToken
     * @custom:error LiquidateCollateralFreshnessCheck is thrown when interest has not been accrued on the collateral seToken
    ...
    function liquidateBorrow(
        ...
        _liquidateBorrow(msg.sender, borrower, repayAmount, seTokenCollateral, false);
        return NO_ERROR;
    }
     ...
     * @custom:error ReduceReservesCashNotAvailable is thrown when the seToken does not have sufficient cash
    ...
    function healBorrow(address payer, address borrower, uint256 repayAmount) external override nonReentrant {
        ...
        if (badDebtDelta != 0) {
            ...
            // We treat healing as "repayment", where seToken is the payer
    ...
    function forceLiquidateBorrow(
        ...
        _liquidateBorrow(liquidator, borrower, repayAmount, seTokenCollateral, skipLiquidityCheck);
    }
     ...
     * @return error Always NO_ERROR for compatibility with Segment core tooling
     * @return seTokenBalance User's balance of seTokens
    ...
    function getAccountSnapshot(
        ...
        view
        override
        returns (uint256 error, uint256 seTokenBalance, uint256 borrowBalance, uint256 exchangeRate)
    {
    ...
    function _liquidateBorrow(
        ...
        uint256 error = seTokenCollateral.accrueInterest();
        if (error != NO_ERROR) {
        ...
        _liquidateBorrowFresh(liquidator, borrower, repayAmount, seTokenCollateral, skipLiquidityCheck);
    }
    ...
    function _liquidateBorrowFresh(
        ...
        if (seTokenCollateral.accrualBlockNumber() != _getBlockNumber()) {
        ...
        if (address(seTokenCollateral) == address(this)) {
            ...
            seTokenCollateral.seize(liquidator, borrower, seizeTokens);
        }
     ...
     *  It's absolutely critical to use msg.sender as the seizer seToken and not a parameter.
     * @param seizerContract The contract seizing the collateral (either borrowed seToken or Comptroller)
     * @param liquidator The account receiving seized collateral
     * @param borrower The account having collateral seized
     * @param seizeTokens The number of seTokens to seize
     */
