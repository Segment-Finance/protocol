...
contract Shortfall is Ownable2StepUpgradeable, AccessControlledV8, ReentrancyGuardUpgradeable, TokenDebtTracker {
    ...
    struct Auction {
        ...
        uint256 startBidBps;
        mapping(SeToken => uint256) marketDebt;
        mapping(SeToken => uint256) bidAmount;
    }
    ...
    function _startAuction(address comptroller) internal {
        PoolRegistryInterface.SegmentPool memory pool = PoolRegistry(poolRegistry).getPoolByComptroller(comptroller);
        ...
        uint256 marketsCount = auction.markets.length;
        for (uint256 i; i < marketsCount; ++i) {
            SeToken seToken = auction.markets[i];
            auction.marketDebt[seToken] = 0;
        }

        delete auction.markets;

        SeToken[] memory seTokens = _getAllMarkets(comptroller);
        marketsCount = seTokens.length;
        ResilientOracleInterface priceOracle = _getPriceOracle(comptroller);
        uint256 poolBadDebt;

        uint256[] memory marketsDebt = new uint256[](marketsCount);
        auction.markets = new SeToken[](marketsCount);

        for (uint256 i; i < marketsCount; ++i) {
            uint256 marketBadDebt = seTokens[i].badDebt();

            priceOracle.updatePrice(address(seTokens[i]));
            uint256 usdValue = (priceOracle.getUnderlyingPrice(address(seTokens[i])) * marketBadDebt) / EXP_SCALE;

            poolBadDebt = poolBadDebt + usdValue;
            auction.markets[i] = seTokens[i];
            auction.marketDebt[seTokens[i]] = marketBadDebt;
     ...
     * @param comptroller Address of the pool's comptroller
     * @return markets The pool's markets as SeToken array
     */
    function _getAllMarkets(address comptroller) internal view returns (SeToken[] memory) {
