...
contract SRTVault is SRTVaultStorage, AccessControlledV5 {
    ...
    /// @notice Event emitted on SRT deposit
    event Deposit(address indexed user, uint256 amount);

    /// @notice Event emitted when accruedInterest and SRT PrincipalAmount is withrawn
    event Withdraw(
    ...
    function initialize(address _srtAddress, uint256 _interestRatePerBlock) public {
    ...
    /**
     * @notice Deposit SRT to SRTVault for a fixed-interest-rate
    ...
    function deposit(uint256 depositAmount) external nonReentrant isInitialized isActive {
        ...
        if (user.userAddress == address(0)) {
            ...
            if (accruedInterest > 0) {
                uint256 srtBalance = srt.balanceOf(address(this));
                require(
                    srtBalance >= accruedInterest,
                    "Failed to transfer accruedInterest, Insufficient SRT in Vault."
                );
                emit Claim(userAddress, accruedInterest);
                srt.safeTransfer(user.userAddress, accruedInterest);
            }
        }
        ...
        srt.safeTransferFrom(userAddress, address(this), depositAmount);
    }

    /**
     * @notice get accruedInterest of the user's SRTDeposits in the Vault
     * @param userAddress Address of User in the the Vault
     * @return The interest accrued, in SRT
     */
    ...
    /**
     * @notice claim the accruedInterest of the user's SRTDeposits in the Vault
     */
    ...
    function _claim(address account) internal {
        ...
        if (accruedInterest > 0) {
            ...
            uint256 srtBalance = srt.balanceOf(address(this));
            require(srtBalance >= accruedInterest, "Failed to transfer SRT, Insufficient SRT in Vault.");
    ...
    /**
     * @notice withdraw accruedInterest and totalPrincipalAmount of the user's SRTDeposit in the Vault
     */
    function withdraw() external nonReentrant isInitialized userHasPosition(msg.sender) isActive {
        ...
        uint256 srtForWithdrawal = accruedInterest.add(totalPrincipalAmount);
        user.totalPrincipalAmount = 0;
        user.accrualStartBlockNumber = getBlockNumber();

        uint256 srtBalance = srt.balanceOf(address(this));
        require(srtBalance >= srtForWithdrawal, "Failed to transfer SRT, Insufficient SRT in Vault.");

        emit Withdraw(userAddress, srtForWithdrawal, totalPrincipalAmount, accruedInterest);
        srt.safeTransfer(user.userAddress, srtForWithdrawal);
    }
    ...
    function _become(ISRTVaultProxy srtVaultProxy) external {
        require(msg.sender == srtVaultProxy.admin(), "only proxy admin can change brains");
        srtVaultProxy._acceptImplementation();
    }
