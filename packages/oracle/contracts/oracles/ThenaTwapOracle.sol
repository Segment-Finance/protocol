...
import { FixedPoint, ThenaAmmOracleLibrary } from "../libraries/ThenaAmmOracleLibrary.sol";
 ...
 */
contract ThenaTwapOracle is AccessControlledV8, TwapInterface {
    ...
    function getPriceCurrent (address asset) public view returns (uint256) {
        TokenConfig memory config = tokenConfigs[asset];
        uint256 priceCurrentMantissa = currentCumulativePrice(config);

        uint256 pairedTokenBaseUnit = config.isBnbBased ? BNB_BASE_UNIT : BUSD_BASE_UNIT;
        uint256 anchorPriceMantissa = (priceCurrentMantissa * config.baseUnit) / pairedTokenBaseUnit;
    ...
    function currentCumulativePrice(TokenConfig memory config) public view returns (uint256) {
        (uint256 price0, uint256 price1, ) = ThenaAmmOracleLibrary.currentCumulativePrices(config.ammPool);
