...
contract ResilientOracle is PausableUpgradeable, AccessControlledV8, ResilientOracleInterface {
    ...
    /// @notice seBNB address
    /// @custom:oz-upgrades-unsafe-allow state-variable-immutable
    address public seBnb;

    /// @notice SEUSD address
    /// @custom:oz-upgrades-unsafe-allow state-variable-immutable
    address public SEUSD;

    /// @notice Set this as asset address for BNB. This is the underlying for seBNB
    ...
    function initialize(
        ...
        address seBnbAddress,
        address SEUSDAddress,
    ...
    function setSeTokens (address seBnbAddress, address SEUSDAddress) external onlyOwner {
        seBnb = seBnbAddress;
        SEUSD = SEUSDAddress;
    }
     ...
     */
    function updatePrice(address seToken) external override {
        address asset = _getUnderlyingAsset(seToken);
    ...
    /**
     * @notice Gets price of the underlying asset for a given seToken. Validation flow:
     ...
     */
    function getUnderlyingPrice(address seToken) external view override returns (uint256) {
    ...
    /**
     * @dev This function returns the underlying asset of a seToken
     * @param seToken seToken address
     * @return asset underlying asset address
     */
    function _getUnderlyingAsset(address seToken) private view returns (address asset) {
        if (address(seToken) == seBnb) {
            asset = BNB_ADDR;
